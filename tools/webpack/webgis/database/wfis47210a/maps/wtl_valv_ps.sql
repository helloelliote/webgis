...
CREATE OR REPLACE VIEW public.viw_wtl_valv_ps
            (id, geom, 레이어, 관리번호, 급수구역, 본관구경, 지관구경, 재질, 변류형식, 회전방향, 총회전수, 현회전수, 구동방법, 변실형태, 변실규격, 설정압력, 제작회사명, 이상상태,
             개폐여부, 읍면동, 법정동, 급수분구, 급수블럭, 설치일자, 공사명, 관리기관, 도엽번호, 관로관리번호, 방향각)
AS
SELECT valv_tb.id,
       valv_tb.geom,
...
FROM wtl_valv_ps valv_tb
         ...
         LEFT JOIN wtl_wtsba_as wbb_tb ON valv_tb.wbb_cde = wbb_tb.wbb_cde
...
WHERE valv_tb.layer = '배기변'::bpchar
   OR valv_tb.layer = '이토변'::bpchar
   OR valv_tb.layer = '제수변'::bpchar
   OR valv_tb.layer = '지수전'::bpchar;
